export const BLUX_EVENT_NAME = 'BLUX_MODAL_OPEN';
export const BLUX_JWT_STORE = '__BLUX__JWT_STORE';
export const RECENT_CONNECTION_METHODS = '__BLUX__RECENT_CONNECTION_METHODS';

export const WC_STELLAR_PUBNET = 'stellar:pubnet';
export const WC_STELLAR_TESTNET = 'stellar:testnet';

// export const BLUX_API = 'https://api.blux.cc';
export const BLUX_API = 'http://localhost:3001';
export const BLUX_APP_ID_HEADER = 'blux-app-id' as const;
export const BLUX_CDN_PATH = 'https://cdn.blux.cc/files';

export const CLOUDFLARE_R2_LOGOS =
  'https://cdn2.blux.cc/core-assets/logos2.json.gz';
